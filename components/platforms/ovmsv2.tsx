...
export async function sendOVMSv2Command(command: { commandCode: CommandCode, params?: any }): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      console.log('[connection OVMSv2 sendOVMSv2Command] command timed out', timeoutId)
    ...
    console.log('[connection OVMSv2 sendOVMSv2Command] PendingCommands size:', PendingCommands.size,
    ...
    if (commandMessage !== undefined) {
      ...
      PendingCommands.set(command.commandCode, pendingCommands);
      // Send the command
      console.log(`[connection OVMSv2 sendOVMSv2Command] Sending command: ${commandMessage}`);
...
function resolveNextPendingCommand(commandCode: number, result: number, parameters: string) {
  console.log('[connection OVMSv2 resolveNextPendingCommand] response', commandCode, result, parameters)
  ...
  if (pendingCommands.length > 0) {
    ...
    console.log('[connection OVMSv2 resolveNextPendingCommand] PendingCommands size:', PendingCommands.size,
    ...
    console.log('[connection OVMSv2 resolveNextPendingCommand] no pending commands for command code', commandCode)
  }
}
...
export function OVMSv2ConnectionIcon(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    var openListener: any;
    var messageListener: any;
    var closeListener: any;

    if (ConnectionIconCount == 1) {
      ...
      messageListener = (event: MessageEvent) => {
        ...
        if (event.data.startsWith('MP-S')) {
          ...
          console.log('[connection OVMSv2] rx event data', event.data)
        }
      }
